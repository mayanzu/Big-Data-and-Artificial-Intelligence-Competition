...
/api/history & page, per\_page, category & 历史数据分页 \\
...
%========================================
\begin{practicebox}[平台实战：用仿真平台数据练习 SQL]
\begin{lstlisting}[language=bash]
# 先从平台获取数据并上传 HDFS
curl -s "http://163.7.1.176:8889/api/export/csv?n=5000" > /tmp/practice.csv
hdfs dfs -put /tmp/practice.csv /user/data/practice.csv
...
CREATE TABLE practice_data (
    timestamp STRING, user_id STRING, event_type STRING,
    session_id STRING, device STRING, ip STRING,
    product_id STRING, category STRING, price DOUBLE,
    page_source STRING, stay_time INT
)
...
\begin{lstlisting}[language=SQL]
-- 练习1：统计各事件类型数量
SELECT event_type, COUNT(*) FROM practice_data GROUP BY event_type;

-- 练习2：筛选购买记录
SELECT * FROM practice_data WHERE event_type = 'buy';

-- 练习3：统计各品类平均价格
SELECT category, AVG(price) FROM practice_data GROUP BY category;
\end{lstlisting}
\end{practicebox}
