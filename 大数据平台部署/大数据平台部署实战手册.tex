...
\begin{tipbox}[准备计划（6-10月，约4个月）]

\textbf{前提}：已学完 Python 基础，了解 Linux 基本命令（cd/ls/vi）

\begin{enumerate}
    \item \textbf{6月}：学 Pandas 数据处理 + 大数据平台部署（重点打基础）
    \item \textbf{7月}：反复练习部署，做到熟练快速；学数据分析
    \item \textbf{8月}：学可视化 + 综合应用（数据挖掘）；做完整模拟
    \item \textbf{9月}：参加网络赛前模拟；查漏补缺
    \item \textbf{10月}：冲刺准备，打印本手册带进考场
\end{enumerate}

\textbf{仿真平台}：\texttt{http://163.7.1.176:8889}——练习用数据源（考试时用题目给的数据，但操作流程一样）。

\textbf{考场提示}：现场赛每队 3 台计算机（一主两从），可以打印材料带入考场。
\end{tipbox}
...
GET & /api/dirty/tsv & 脏数据TSV格式 \\
GET & /api/dirty/enhanced & 增强版脏数据 \\
GET & /api/timeseries & 时间序列统计 \\
GET & /api/hive/ddl & 自动生成Hive建表SQL \\
GET & /api/dirty/csv & 脏数据CSV格式 \\
GET & /api/join & 三表关联数据 \\
GET & /api/export/csv & 批量CSV导出 \\
GET & /api/stats & 统计汇总 \\
\bottomrule
\end{tabular}
}
...
%========================================
\section{大数据平台参数优化（本科独立考核模块）}

\begin{warnbox}[重要提醒]
参数优化是本科组的独立考核模块！考试中可能会要求：
\begin{itemize}
    \item 修改 HDFS 副本数（dfs.replication）
    \item 修改 Spark Executor 内存
    \item 修改 Kafka 分区数
    \item 修改 HBase 并发参数
\end{itemize}

\textbf{修改后一定要验证}：重启服务 + 检查配置是否生效。
\end{warnbox}
...
\begin{tipbox}[配套练习]
本手册的配套练习见各章节内的动手练习框，建议按章节顺序边学边练。
\end{tipbox}
