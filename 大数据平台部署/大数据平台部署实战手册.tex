...
\section{Flink 实时计算引擎（高职要求）}

\begin{knowledgebox}[Flink 是什么？]
Flink 是一个流处理框架，和 Spark Streaming 类似但更强大。在高职竞赛中，Flink 要求出现在"大数据平台搭建与运维"模块中。

\textbf{和 Spark 的区别}：
\begin{itemize}
    \item Spark Streaming：微批处理（每几秒处理一批数据）
    \item Flink：真正的逐条处理（每条数据来了就处理）
\end{itemize}

\textbf{竞赛中你需要做的}：下载、安装、配置、启动 Flink，然后写一个简单的 WordCount 程序验证。
\end{knowledgebox}
...
\begin{lstlisting}[language=bash]
# 下载 Flink
wget https://repo.huaweicloud.com/apache/flink/flink-1.17.2/flink-1.17.2-bin-scala_2.12.tgz
...
# 关键配置
jobmanager.rpc.address: node1    # JobManager 地址
taskmanager.memory.process.size: 1024m  # TaskManager 内存
\end{lstlisting}

\begin{lstlisting}[language=bash]
# 启动 Flink（standalone 模式）
start-cluster.sh
