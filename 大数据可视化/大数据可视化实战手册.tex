...
\subsection{步骤3：撰写分析报告}

\begin{knowledgebox}[竞赛报告结构模板]
\begin{enumerate}
    \item \textbf{问题描述}（1段）：我们分析了什么问题，为什么重要
    \item \textbf{数据说明}（1段）：数据来源、数据量、字段含义
    \item \textbf{分析方法}（1段）：用了什么工具，做了哪些分析
    \item \textbf{分析结果}（核心，配图表）：每张图配 2-3 句解读
    \item \textbf{结论建议}（1段）：根据分析结果给出建议
\end{enumerate}
\end{knowledgebox}
